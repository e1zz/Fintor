...
def save_comprobante(comprobante: dict, *, tenant, download_request, tipo: str):
    ...
    xml = _decode_content(comprobante.get('content'))
    defaults = {
        'tenant': tenant,
        'sat_download_request': download_request,
        'document_type': _document_type(tipo),
        'sender_rfc': (comprobante.get('rfCemisor') or comprobante.get('rfcEmisor') or '')[:13],
        'sender_name': (comprobante.get('razonSocialEmisor') or '')[:255],
        'receiver_rfc': (comprobante.get('rfcReceptor') or '')[:13],
        'receiver_name': (comprobante.get('razonSocialReceptor') or '')[:255],
        'issue_date': _parse_date(
            comprobante.get('fechaEmision') or comprobante.get('fechaCertificacion')
        ),
        'total': _parse_decimal(comprobante.get('total')),
        'document_subtype': (comprobante.get('efectoDelComprobante') or '')[:1] or None,
        'status': (comprobante.get('estadoDeComprobante') or comprobante.get('estatusCancelacion') or '')[:20] or None,
        'xml_content': xml,
    }
